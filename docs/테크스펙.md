...
| 5-6주 | 온디바이스 얼굴 인식 귀속 판정 테스트케이스 통과율 측정 |
...
- ~~교사는 서비스에 등록할 원아의 법정대리인을 통해 별도 동의를 받아야함~~ → **2026-09-22 결정으로 폐기.** 동의 수집은 더 이상 전제조건(서면)이 아니라 **서비스 안에서** 합니다 — 학부모가 **원아별 초대 링크**로 가입하면서 동의 항목에 직접 동의합니다 (FR-28). 교사가 서면 동의 결과를 대신 등록하지 않습니다
 (동의 항목: 자녀 개인정보 수집·이용, 얼굴 특징정보 처리, 원 활동 사진·영상 촬영 및 공유 동의, 기존에 촬영된 원아사진 활용동의(가칭))
 - **(미정)** 초대 링크의 발급 주체·형식·만료, 동의 철회 주체(FR-22) — `docs/open-questions.md`
- 학부모는 위의 동의를 거부가능 (거부할 수 있는 것은 아래 ③뿐)
- **동의 항목별 등급 (2026-09-13 결정)**
  - **① 개인정보 수집·이용: 필수.** 동의하지 않으면 서비스를 이용할 수 없습니다.
  - **② 활동 사진·영상 촬영 및 S3 업로드: 필수.** 동의하지 않으면 서비스를 이용할 수 없습니다. 사진의 S3 업로드 자체는 이 동의로 커버됩니다.
  - **③ 얼굴 특징정보(임베딩) 서버 등록: 선택.** 미동의 시 임베딩을 등록하지 않아 얼굴 자동 분류 대상에서 빠지지만, **알림장·관찰일지 서비스는 그대로 이용할 수 있습니다.** 해당 원아는 교사의 수동 분류로 다룹니다.
  - **번호는 `ConsentRecord.consent_type`과 1:1입니다** — ①`개인정보수집이용` ②`활동사진영상촬영` ③`얼굴특징정보처리` (2026-09-20 확정). 거부할 수 있는 것은 ③뿐이므로, **거부 케이스는 ③ 하나만 존재합니다.**
  - 미동의 원아가 귀속된 사진은 S3에는 올라가되 **행동 분석(LLM 근거)에는 쓰지 않습니다** (아래 "공통 규칙").
...
| ID | 요구사항 | 사유/비고 |
| --- | --- | --- |
| FR-01 | ~~교사는 원아의 동의 미동의 여부를 등록가능하다~~ | **09/22 폐기.** 동의 주체가 학부모로 바뀌어 교사가 등록할 일이 없어졌습니다 (FR-28). ID는 공란으로 두지 않고 취소선으로 남깁니다 — 하위 문서가 참조하므로 **재사용하지 마세요** |
| FR-02 |  |  |
| FR-03 | 교사는 동의를 하지 않은 원아를 확인가능 | 09/22 이후에도 유지. 교사가 등록한 값이 아니라 **학부모가 앱에서 남긴 동의 결과**를 확인합니다 |
| FR-04 | 사진·영상 업로드 후 아이별 자동 분류 (온디바이스 얼굴 인식) | 09/15 결정: 귀속 판정은 온디바이스 얼굴 인식으로만 합니다. 호명 발화는 귀속에 쓰지 않고 초안 작성 근거로만 사용합니다 |
...
| FR-23 | 교사는 가입 시 어린이집 가입 코드(`Center.center_code`)로 소속 어린이집을 선택할 수 있다 | 09/13 추가 |
| FR-24 | 어린이집 가입 코드가 아직 없으면, 교사가 어린이집 정보를 직접 입력해 어린이집과 가입 코드를 생성할 수 있다 | 09/13 추가. FR-23의 선행 조건이 없는 첫 교사용 |
| FR-25 | 교사는 소속 어린이집의 반을 선택하거나 새로 생성해 담당 반으로 지정할 수 있다 | 09/13 추가. 기존 "온보딩 1 — 반 만들기"에 선택 경로 추가 |
| FR-27 | 수집한 맥락(사진 맥락·STT 발화 등)을 조합해 **원아별 하루 일과**를 교사에게 제공한다 | 09/19 추가. 초안과 별개의 산출물이며, 초안 생성의 입력으로도 쓰인다(처리 파이프라인 5·6단계) |
| FR-26 | 생성된 초안에는 작성한 교사의 이름이 포함된다 | 09/13 추가. 작성자는 `DraftDocument.author_teacher_id`로 기록하고, 이름은 서버가 렌더링 단계에서 붙임(H-2 — LLM 프롬프트에 교사 실명을 넣지 않음) |
| FR-28 | 학부모는 원아별 초대 링크로 가입하면서 동의 항목(①②③)에 직접 동의한다 | 09/22 추가·결정. 동의 주체가 교사(서면 등록)에서 학부모(앱)로 바뀌며 FR-01을 대체합니다. 링크로 반 참여를 유도하므로 교사가 따로 등록할 필요가 없습니다. **(미정)** 링크 발급 주체·형식·만료, FR-22 철회 주체 |
|  |  |  |
...
| 기능 | 영상에 미동의 원아가 있는지 교사가 확인할 때 쓰는 보조 기능 | 09/19 추가. 확인 자체는 교사가 직접 합니다(§공통 규칙) — 교사 부담이 커서 돕는 수단만 could로 둡니다 |
...
| NFR-03 | **학부모 접근**은 그 원아의 졸업 후 1년까지 제공되며, 이후 조회 시점에 차단된다. 기준값은 `Child.graduated_at`. | 서버비와 사용자 경험(내 아이의 추억이 사라졌어)의 균형 맞추기 |
| NFR-03-b | **파기**는 그 사진에 귀속된 원아가 **전원 만료된 사진만** 배치로 삭제한다. 한 명이라도 기한이 남아 있으면 삭제하지 않는다. | 09/19 결정. 한 사진에 졸업·퇴소일이 다른 원아가 여럿이면 만료일이 한 값으로 정해지지 않아 접근 차단과 파기를 분리했습니다. 그래서 `MediaAsset.retention_expires_at`을 두지 않고 파기 배치가 조인해서 계산합니다 |
...
| NFR-13 | `develop`에 머지된 코드는 사람이 서버에 직접 접속하지 않고 자동으로 배포 서버에 반영된다. 배포 대상 브랜치는 `develop`. | 멘토 리뷰(09/20). 서버가 1대라 개발용·운영용 분리가 불가능하므로 `develop`을 배포 기준으로 삼아 개발 내용을 빠르게 확인한다. `main` 배포 도입 시점은 미정 |
| NFR-14 | CI는 AWS 장기 자격증명(액세스 키)을 저장하지 않고 OIDC 단기 토큰만 사용한다. | 캠퍼스 AWS 환경이 IAM 사용자·액세스 키 생성을 차단한다. 키가 저장소·CI에 남는 유출 경로를 만들지 않기 위함 |
| NFR-15 | 배포된 버전은 Git 태그로 식별되며, 그 태그의 코드로 재배포해 이전 상태로 되돌릴 수 있다. | 멘토 리뷰(09/20). 디스크 확보를 위해 서버의 이전 이미지를 정리해도 롤백 경로가 남도록 |
...
| --- | --- | --- | --- | --- |
| 0. 등록 임베딩 캐시 조회 | 코드(서버 필터링) + 온디바이스(캐시)  | 배치 시작 시 서버가 **담당 반의 재원 원아 중 동의가 유효하고 임베딩이 등록된 원아**의 기준 임베딩만 브라우저로 전달. 동의 수집은 학부모가 앱에서 직접 하지만(FR-28) 철회 시 임베딩을 즉시 삭제하므로(FR-22) 평소 두 조건은 같은 집합임. 그럼에도 **조회는 동의 레코드와 임베딩을 함께 확인**함 — 철회 처리가 중간에 실패해 임베딩이 남더라도 미동의 원아가 대조 대상에 들어가지 않도록, 두 값이 어긋나면 **제외되는 쪽으로 실패**시키기 위함. 등록된 원아라도 임베딩이 없으면 대상에서 빠짐. 브라우저는 이번 배치 동안만 메모리에 캐시. 임베딩 조회 실패 시 처리 중단 | 이번 배치용 기준 임베딩 캐시 |  |
| 1-A. 근거 수집 — 얼굴 | 온디바이스 모델 (코드가 호출) | MediaPipe BlazeFace 얼굴 검출 → ArcFace 임베딩 → 기준 임베딩과 비교 / frame_extract 근거 프레임 선택. 얼굴 실패 시 미분류함(블러는 하지 않습니다 — 09/13 폐기) | 아이별 얼굴 매칭 결과, 근거 프레임 | H-3. 얼굴은 온디바이스 유지 |
| 1-B. 근거 수집 — 음성(STT) | **외부 STT 서비스 (서버가 호출)** | 오디오(음성메모·영상 오디오)를 S3에 올린 뒤 서버가 외부 STT API를 호출해 발화 텍스트를 받아 `TranscriptSegment`로 저장. 인식 실패 시 "멘트 없음". **오디오는 자르지 않고 통째로 올립니다** — 브라우저 VAD는 쓰지 않고 발화 구간 판정은 외부 STT에 맡깁니다 (09/14 결정). **(미정)** 사용할 STT 제공자 | 발화 텍스트 | 2026-09-13 결정: 온디바이스 whisper-base → 외부 STT. 소음 환경 정확도가 온디바이스 모델로는 확보되지 않아서. LLM에 가는 것은 3단계에서 비식별화된 텍스트라 H-2는 그대로 지켜집니다(오디오는 LLM에 가지 않음). 다만 **오디오 자체가 외부 처리자에게 나가므로 위탁 처리 계약·동의서 범위 확인이 필요**합니다 |
| 2. 귀속 판정 | 코드 (규칙 기반, **온디바이스**) | 1-A의 얼굴 매칭 결과로 귀속을 판정. **호명 발화는 귀속에 쓰지 않습니다** (09/15 결정 — 호명은 초안 작성 근거로만 사용). 신뢰도 임계값 미달 시 자동 결정하지 않음. **한 사진에 여러 아이가 있을 때 한 명이라도 미등록·애매하면 그 사진 전체를 "미등록"으로 표시**하고, 확실한 아이는 선택된 상태로 둔 채 애매·미등록 아이만 교사가 고르게 합니다. 교사가 선택을 마치면 **선택된 모든 아이에게 귀속**됩니다 (09/13 결정, FR-14) | 아이별 귀속 결과, 미확정 건은 미분류함 | 09/15 결정으로 온디바이스 유지. 교사 검수를 마친 확정 결과만 서버로 전송됨 |
...
| 5. 에이전트1 — 근거 수집 | LLM + tool | 발화 내용에 따라 tool 호출 (활동계획 조회 / 이전 기록 조회 / 원아 명단 조회). **해당 아이의 사진도 근거에 포함**(NFR-09) — 발화가 1차, 사진은 맥락 보강. **세부 처리로 '사진 맥락 추출'을 둡니다** — 사진에서 보이는 행동·사물·상황을 뽑아 발화 근거를 보강하며, 별도 파이프라인 단계가 아니라 이 단계 안의 처리입니다. 보이지 않는 감정·의도·발달 수준은 사실로 쓰지 않고, 다인원 사진에서 행동 주체가 불분명하면 특정 원아에게 귀속하지 않습니다. 사진은 S3의 `MediaAsset` 원본을 서버가 읽어 전달(별도 사본 없음). 근거 부족 시 재호출, 그래도 부족하면 미분류함 | 조합된 근거 묶음(비식별 텍스트 + 사진), **아이별 하루 일과** | FR-27. 근거를 조합해 하루 일과를 먼저 만들고, 6단계는 원본 근거 전체가 아니라 이 하루 일과를 입력으로 받습니다 — LLM 토큰 사용량을 줄이기 위함 (09/19 결정) |
| 6. 에이전트2·3 — 초안 생성 | LLM + tool (병렬) | **5단계가 만든 하루 일과를 입력으로** 초안을 생성합니다. 에이전트2: 영역 스타일 조회(표준보육과정 6대 영역)·발달 지침 조회 → 관찰일지. 에이전트3: 아이 감정 어휘 조회·발달 문안 조회 → 알림장 | 관찰일지·알림장 초안 2종 | FR-26: 작성 교사는 코드가 `author_teacher_id`로 기록. 교사 실명은 프롬프트에 넣지 않음 |
| 7-A. 코드 검증 | **코드** (최상위 `tools/verification/`) | 출력의 필수 항목이 있는지, 문장에 붙은 근거 ID가 **실제로 존재**하는지, 그 근거가 **대상 원아·날짜와 맞는지** 검사. 근거 없는 문장과 잘못된 참조를 구분해 반환. **여기서 걸리면 Critic을 호출하지 않고 바로 재생성** | 코드 검증 결과 | AI 회의 추가. 규칙으로 판정되는 것을 LLM에 맡기지 않기 위함이고(책임 분담 표), LLM 호출도 아낍니다 |
| 7-B. 에이전트4 — 검증(Critic) | LLM (별도 컨텍스트) | **7-A를 통과한 초안만** 대상. 문장의 **내용**이 근거로 뒷받침되는지 대조(과장·추측·타 아이 행동 혼입), 금지어 검사, 영역 스타일 검사. 두 문서 모두 필수 통과. 실패 시 재생성, 상한 초과 시 미분류함. **Critic 자체의 출력 오류도 정상 통과와 구분합니다** | 검증 통과 초안 |  |
...
| 흐름 | 트리거 | 빈도 | AI 관여 |
| --- | --- | --- | --- |
| A. 가입·소속 설정 | 교사의 회원가입 | 교사당 1회 | 없음 |
| B. 원아 얼굴 등록 | 교사의 원아 얼굴 사진 업로드 | 원아당 1회 | 임베딩 생성만 |
| C. 일지 생성 (위 처리 파이프라인) | 교사의 촬영분 업로드. 마지막 귀속 저장이 끝나면 **FE가 `POST /classes/{class_id}/jobs`를 한 번 호출**해 시작 (09/27 결정) | 매일 | 있음 |
| D. 학부모 열람 | 승인본 노출 이후 | 상시 | 없음 |
| E. 공지 게시판 | 교사의 공지 등록 | 비정기 | 없음 |
| F. 보관·파기 | 스케줄 / 파이프라인 종료 | 배치 | 없음 |

### A. 가입·소속 설정 흐름 (교사당 최초 1회)

| 단계 | 처리 주체 | 처리 내용 | 참고한 요구사항 |
| --- | --- | --- | --- |
| A-1 | 교사 | 역할(교사)을 선택하고 이름·이메일·비밀번호 입력 | FR-13 |
| A-2 | 교사 | 어린이집 가입 코드 입력 → 코드로 확인된 어린이집 이름을 보고 소속 확정 | FR-23 |
| A-3 | 코드 | 가입 코드 검증. 없는 코드면 "직접 등록" 경로 안내 | FR-23, FR-24 |
| A-4 | 교사 | (코드가 없을 때) 어린이집 이름·주소를 입력해 `Center` 생성 → 서버가 가입 코드 발급 | FR-24 |
| A-5 | 교사 | 소속 어린이집의 반 목록에서 담당 반을 **선택**하거나, 반 이름·연령 구분을 입력해 **생성** | FR-25 |
| A-6 | 코드 | 선택한 반에 교사를 담당자로 연결(`Class.teacher_id`). 이미 다른 교사가 담당인 반의 처리는 **(미정)** | FR-25 |
| → |  | 이후 B 흐름(원아 명단·동의·얼굴 등록) |  |
...
| B-4 | 코드 | **학부모가 원아별 초대 링크로 가입하며 남긴 동의**를 원아 단위로 저장 (09/22, FR-28). 처리 파이프라인 0단계(동의확인 게이트)의 조회 대상 | 동의 대장 | FR-28, FR-03, 처리 파이프라인 0단계 |
...
| C-3 | 코드(서버) | 업로드 완료된 오디오(음성메모·영상 오디오)를 외부 STT 서비스로 보내 `TranscriptSegment` 생성 | 파이프라인 1-B, 09/13 결정 |
| → |  | 이후 처리 파이프라인 0~9단계 |  |
...
| F-3 | 코드 | 학부모 접근은 그 원아의 졸업 후 1년까지. 조회 시점에 `Child.graduated_at` + 1년으로 판정한다. 파기는 귀속 원아가 전원 만료된 사진만 배치로 삭제(NFR-03-b). (미정) 그 기간 안에서 원본 → 저화질 전환 시점 | NFR-03 |
...
- **미동의 원아가 귀속된 사진은 행동 분석(LLM 근거)에 쓰지 않습니다** (2026-09-13 결정) — B·C 흐름 해당
  - 사진의 S3 업로드 자체는 필수 동의(② 활동사진영상촬영)로 커버되므로 **업로드는 합니다.** LLM으로 나가는 경로에서만 제외합니다.
  - **얼굴 블러는 쓰지 않습니다.** 09/13 스파이크 결과 인식(동의/미동의 구분)은 잘 되지만 **블러 영역이 과도하게 커져 엉뚱한 곳까지 가립니다.** 영상은 재인코딩 부담까지 겹칩니다.
  - 판정 경로: ③ 동의(얼굴특징정보처리)를 하지 않은 원아는 **얼굴로 식별되지 않습니다.** 그래서 미등록·애매한 얼굴이 있는 사진은 교사 수동 분류로 넘기고(FR-14), **교사가 그 사진에 미동의 원아를 귀속시키면 그 사진을 행동 분석에서 제외**합니다. 자동 분류만으로는 판정할 수 없습니다.
  - 전원 동의로 귀속이 끝난 사진은 다인원이어도 그대로 LLM에 전달합니다.
  - **영상은 미동의 원아가 찍혔는지를 교사가 직접 확인합니다** (09/19 결정). 프레임 단위 자동 판정은 잠깐 스쳐 지나간 원아를 놓치기 때문입니다. 교사 부담을 더는 보조 기능은 could에 있습니다.
  - 추후 방향(교사에게 직접 사진 편집 지시, 블러 재시도)은 `docs/open-questions.md`에 있습니다.
...
| 구분 | 범위 |
| --- | --- |
| 코드 (규칙 기반) | 동의확인 게이트, 귀속 판정(온디바이스), 비식별화, 오케스트레이션, 학부모 노출 |
...
| Center(원) | id, center_code, name, address, contract_status, created_by_teacher_id | contract_status: 체험/정식 계약 상태. center_code는 교사 가입 시 소속을 고르는 **가입 코드**로 UNIQUE(FR-23). 코드가 없는 어린이집은 교사가 이름·주소를 입력해 만들고 서버가 코드를 발급하며, 이때 contract_status는 `체험`으로 시작(FR-24). created_by_teacher_id는 그렇게 만든 교사(널 허용) |
| Class(반) | id, center_id, teacher_id, name, age_group | 예: 만3세반. teacher_id로 담당 교사 참조 (반은 항상 교사 1명). 교사는 같은 center의 기존 반을 선택하거나 새로 만들 수 있음(FR-25) |
...
| Child(원아) | id, class_id, name, birth_date, status, enrolled_at, graduated_at | status: 재원/졸업/퇴소. graduated_at은 졸업·퇴소 확정일로, NFR-03의 "졸업 후 1년" 계산 기준값(접근 차단 판정과 NFR-03-b 파기 배치가 이 값을 씁니다). 재원 중에는 null |
...
| 엔티티 | 필드 | 설명/비고 |
| --- | --- | --- |
| ConsentRecord(학부모(법정대리인) 동의 기록 | id, child_id, parent_id, consent_type, status, agreed_at, revoked_at | consent_type: `개인정보수집이용`(①) / `활동사진영상촬영`(②) / `얼굴특징정보처리`(③) 동의 여부(사전 동의여부). **①②는 필수, ③은 선택**입니다 (09/13 등급 결정, 09/20 번호 확정 — 전제조건 참고). 번호는 화면·회의에서 부르는 이름이고 DB에는 문자열 값이 들어갑니다. 얼굴특징정보처리 미동의 원아도 서비스는 이용하며, 그 원아가 귀속된 사진만 LLM 근거에서 빠집니다. 법적 요구사항 문구는 요구사항 섹션 상단 이미지 기준으로 확정 필요. **09/22부터 동의 주체는 학부모 본인입니다** — `parent_id`는 초대 링크로 가입해 직접 동의한 학부모 계정이며, 교사가 대신 등록하지 않습니다 (FR-28) |
...
| 엔티티 | 필드 | 설명/비고 |
| --- | --- | --- |
| MediaAsset(교사가 촬영해서 올린 원본 파일) | id, client_photo_id, class_id, teacher_id, type, captured_at, storage_url, size_bytes, storage_tier, ~~redacted~~, model_version, llm_allowed, proxy_url, thumbnail_url, derivative_state | client_photo_id는 클라이언트가 생성한 UUID로 UNIQUE 제약. 네트워크 실패로 재전송돼도 중복 저장되지 않음(`ON CONFLICT DO NOTHING`). ~~redacted~~: 로컬 블러 처리 여부. **09/13 블러 폐기로 사용하지 않습니다 — 마이그레이션에 넣지 않으며, 블러 기능이 부활하면 그때 추가합니다** (09/21). model_version: 로컬 분류에 쓰인 모델 버전(교사 기기마다 다를 수 있음). storage_url의 객체 하나를 학부모 열람·다운로드(FR-09/10)와 초안 맥락 추출(NFR-09) 양쪽이 공유한다. 같은 원본을 용도마다 따로 업로드해 2벌 보관하지 않는다 — 다만 **원본에서 다시 만들 수 있는 파생본은 예외**다(아래 proxy_url·thumbnail_url). storage_tier: original/degraded/deleted. 보관 기한은 졸업 후 1년(NFR-03). **만료일 컬럼은 두지 않고 파기 배치가 `MediaChildLink`–`Child`를 조인해 계산한다**(NFR-03-b). size_bytes: 업로드 완료 통지를 검증할 때 S3에서 확인한 실제 크기를 그대로 남긴다 (09/21). llm_allowed: 이 파일을 LLM 경로에 넘겨도 되는지. **boolean NOT NULL, default false** (09/21). 교사가 검수에서 확정해야 true가 되며, 옆 반 아이·외부 성인·미동의 원아가 남아 있으면 false. agents로 넘기는 게이트가 이 값을 검사한다 (H-2). proxy_url·thumbnail_url: 원본에서 만든 파생본 주소(nullable), 재생·미리보기용 (FR-07, 09/21). derivative_state: 파생본 생성 상태(pending/ready/failed). **(미정)** 변환 실행 위치 — ffmpeg 의존성·Celery 범위와 함께 결정. (미정) 기한 안에서 원본 → 저화질 전환 시점과 저화질 보관 방식(카카오톡 참고) |
| MediaChildLink(사진-아이 매칭 결과) | id, media_id, child_id, method, confidence_score | 교사 검수를 마친 확정 결과만 기록. 미분류·미검수 상태는 로컬에만 존재하며 서버로 전송되지 않음. method: `face_recognition`(로컬 자동) / `manual`(로컬 수동), NOT NULL (09/21 확정. 09/15 호명 교차검증 폐기로 기존 `face_name_cross_check`는 폐기). confidence_score: `face_recognition`일 때만 채우고 `manual`이면 null. 미식별 얼굴이 함께 찍힌 경우 교사 수동 분류로 넘기고, 교사가 귀속시키지 않은 인물의 행은 생성되지 않음 (09/13 블러 폐기) |
| TranscriptSegment(영상 속 음성 stt 텍스트화 결과) | id, media_id, start_time, end_time, raw_text, source | source: video_audio/voice_memo. 음성 인식 실패 시 raw_text = "멘트 없음". 09/13부터 **서버가 외부 STT API를 호출해 채웁니다**(파이프라인 1-B). `raw_text`에는 실명 호명이 그대로 들어 있으므로 agents로 넘기기 전 비식별화(3단계)를 반드시 거칩니다 (H-2) |
...
| 엔티티 | 필드 | 설명/비고 |
| --- | --- | --- |
| GenerationJob(반·날짜 단위 초안 생성 요청 1건) | id, class_id, record_date, request_id, requested_by_teacher_id, created_at, updated_at | FE가 폴링하는 `job_id`는 이 레코드의 id. 원아별 `Job`을 묶는 부모이고, 반 전체 status·stage·진행률은 원아별 Job에서 계산해 따로 저장하지 않음. `request_id`는 UNIQUE(같은 값으로 다시 보내면 기존 레코드 반환). `requested_by_teacher_id`는 세션 교사로, 비동기 task가 초안의 `author_teacher_id`를 채우는 데 씀(FR-26). 같은 반·날짜에 진행 중인 요청이 있으면 새로 만들지 않음(`JOB_ALREADY_RUNNING`). 원아별 Job에 `child_id`를 NULL로 풀지 않으려고 부모를 따로 둠 (09/27 결정) |
| Job(`orchestrate_drafts` 요청 1건을 추적하는 레코드) | id, generation_job_id, child_id, target_date, status, retry_count, created_at, updated_at | 원아 한 명의 처리 단위. `generation_job_id`는 부모 GenerationJob FK. Celery task는 재시도마다 새 task ID를 받지만 같은 요청이면 Job은 하나. target_date는 EvidenceBundle.date를 채우는 데 씀(부모의 record_date와 같은 값). status: pending/running/succeeded/failed |
| EvidenceBundle(한 아이의 하루치 근거) | id, child_id, date, media_refs, transcript_refs, context_lookup, source_activity_plan_id | 에이전트1 산출물. context_lookup: 활동계획/이전기록/원아명단 조회 결과 요약. **`developmental_guideline`·`teacher_persona` 두 고정 키를 갖는 JSON**입니다 — 키를 고정해야 에이전트2·3이 조회 결과를 구조로 읽습니다. `source_activity_plan_id`는 **FK 없이 UUID nullable**이고 코드에 TODO를 남깁니다 — 활동계획 테이블이 아직 확정되지 않아 지금 FK를 걸면 마이그레이션을 되돌려야 합니다 (09/19 결정) |
| DraftDocument(EvidenceBundle을 바탕으로 실제 생성된 문서 - 아이 한 명을 기준으로 알림장/일지 2건 생성) | id, child_id, evidence_bundle_id, author_teacher_id, doc_type, content, status, created_at, approved_at | author_teacher_id: 초안을 만든 교사(FR-26). 담임이 바뀌어도 작성 시점 교사가 남도록 `Child → Class.teacher_id`로 유추하지 않고 별도 컬럼에 기록. 표시용 이름은 서버가 이 FK로 조회해 붙이며, LLM 프롬프트에는 넣지 않음(H-2). doc_type: `observation_log`(관찰일지)/`parent_note`(알림장). status: draft→verified→approved. **status가 unclassified가 되면 UnclassifiedItem에 ref_type=draft인 행이 함께 생성됩니다**, FR-06 |
| SentenceEvidence(DraftDocument안의 문장 하나하나가 어떤 근거에서 나왔는지 연결) | id, draft_id, sentence_index, evidence_id, source_media_id, source_timestamp, source_text | 문장별 근거(FR-07). 근거 클릭 시 원본 영상 3초 재생에 사용. **한 문장이 근거를 여러 개 가질 수 있으므로 같은 `(draft_id, sentence_index)`에 여러 행이 들어옵니다.** `evidence_id`(tools.contracts.EvidenceItem.evidence_id)를 함께 저장하고 `(draft_id, sentence_index, evidence_id)`에 UNIQUE를 걸어 같은 근거 중복 삽입을 막습니다 (09/22 결정). `source_media_id`·`source_timestamp`는 **nullable**입니다 — 활동계획 근거는 media가 없고, 사진 근거에는 시간 구간이 없습니다 |
| VerificationResult(파이프라인 7단계에서 초안을 검증한 로그) | id, draft_id, check_type, sentence_index, result, detail, checked_at | **7-A 코드 검증과 7-B Critic 결과를 같은 테이블에 남깁니다** — 재생성 판정이 둘을 함께 보기 때문입니다. check_type: 7-A는 `근거존재`/`대상일치`/`출력형식`, 7-B는 `근거대조`/`금지어`/`타아이언급`/`영역스타일`. 문장 단위 검사는 sentence_index로 기록하고, 문서 전체 검사(출력형식·영역스타일)는 null. 관찰일지·알림장 모두 필수 통과 |
| DraftDecisionLog(초안 ID·버전별 최종 판정 로그) | id, draft_id, doc_version, decision, reason, regeneration_count, critic_retry_count, checked_at | decision: pass/regenerate/retry_critic/needs_teacher_review. VerificationResult는 개별 검사 실패 사유를 담고 이 테이블은 그걸 종합한 최종 판정을 담아 서로 대체하지 않음. critic_retry_count는 재생성 횟수(regeneration_count)와 별도로 셈 — Critic 응답 오류 재시도는 재생성이 아님 (09/22 정은-한상균 합의, docs/ai-data-contract.md) |
...
erDiagram
  ...
  GENERATIONJOB ||--o{ JOB : groups
  JOB ||--o{ EVIDENCEBUNDLE : triggers
  ...
  DRAFTDOCUMENT ||--o{ DRAFTDECISIONLOG : decided_by
...
| ~~blob_redacted~~ | - | **09/13 블러 폐기로 사용하지 않음** |  |
...
| excluded_reason | 문자열(열거형) | 제외 사유. `미동의포함`은 09/13 블러 폐기 정리로 **제외 사유가 아닙니다** — 업로드하고 서버가 LLM 경로에서 뺍니다 | `부적절` / `기타` |
| upload_state | 문자열(열거형) | 전송 상태 | `대기` / `전송중` / `확인됨` / `실패` |
| attempt_count | 숫자 | 재시도 횟수 | 2 |
| server_media_id | UUID | ack로 받은 서버 id |  |

### 영상·음성 메모 — 로컬 처리하지 않음 (2026-09-14 결정)

**위 `LocalBatch`·`LocalPhoto` 구조는 사진 전용입니다.** 영상과 음성 메모는 브라우저에서 분류·검수·편집하지 않고 **업로드 큐만 거쳐 S3로 바로 올립니다.**

- 영상 프레임 추출·얼굴 분류·재인코딩을 브라우저에서 하지 않습니다. 조각 전송 여부는 업로드 구현 문제일 뿐 처리 흐름과 무관합니다.
- 음성 메모는 교사 검수 대상이 아닙니다. 올라간 오디오는 서버가 외부 STT로 보냅니다 (파이프라인 1-B).
- **VAD를 브라우저에서 돌리지 않습니다.** 오디오를 자르지 않고 통째로 올리고, 발화 구간 판정은 외부 STT 서비스에 맡깁니다. 아래 "온디바이스 AI 모델 인터페이스"에서 Silero VAD가 빠지는 근거입니다.
- **영상에 미동의 원아가 담긴 경우도 사진과 같습니다.** S3 업로드는 필수 동의(②) 범위라 그대로 올리고, 보호는 LLM 경로 제외로 합니다 (위 "공통 규칙", H-2). 전체 제외·구간 편집은 채택하지 않습니다.

<!-- 2026-09-15: 귀속 판정은 온디바이스 얼굴 인식 단독으로 확정. 호명 발화는 귀속에 쓰지 않는다.
     영상은 브라우저에서 프레임을 뽑지 않으므로 얼굴 기준 자동 귀속 대상이 아니며, 귀속이 필요하면
     교사 수동 분류(FR-14)로 처리한다. -->

**공통 — 아직 정하지 않은 것**: 한 배치에 사진·영상·음성 혼재를 허용할지

<!-- 2026-09-27: 일지 생성 트리거는 FE 호출로 결정(agents 담당 정은). 서버는 업로드를 파일마다 따로 받아
     마지막 파일을 알 수 없어서, media 완료 통지의 .delay()로 시작하면 파일마다 시작되거나 대기 시간으로
     추측해야 한다. 창을 닫아 호출이 빠지면 오늘의 기록 재진입(GET /classes/{class_id}/jobs?record_date=)에서 복구한다.
     남은 것: 재진입 시 media_ids 출처, 근거 범위(media_ids vs 날짜 전체), STT 미완료 시 처리. -->
...
## [아이담 API 문서](https://app.notion.com/p/3e42bb98425780a3bb6af1388b145640)

엔드포인트별 요청·응답 예시와 화면별 API 목록을 담습니다.

## 공통 API 규약

> `backend/CLAUDE.md`에서 옮겨왔습니다 (2026-09-18). 코드가 곧 명세가 되는 내용을 매 세션 로드되는 CLAUDE.md에 두지 않기 위함입니다 (`docs/doc-management.md` §원칙 ⑦).
> **코드가 생긴 뒤 이 표와 `/docs`의 OpenAPI가 갈리면 OpenAPI가 맞습니다.** 엔드포인트 목록은 여기에 손으로 유지하지 않습니다.
> 위 "요청/응답 구조·예시"(노션)는 여전히 저장소 미반영입니다 — 이 절은 **형식 규약**만 다룹니다.
...
| 날짜만 있는 값 | `YYYY-MM-DD`. **한국 시간(KST) 기준 하루**입니다 — `record_date`처럼 "그날의 기록"을 가리키는 값이 대상입니다 (2026-09-23 추가) |
...
**공통 에러 코드** (도메인별 코드는 각 도메인 문서에. 2026-09-23 추가)

| 코드 | 상태 | 뜻 | 프론트 동작 |
|---|---|---|---|
| `UNAUTHENTICATED` | 401 | 로그인 상태가 아님 | 로그인 화면으로 보냅니다. 단 로그인 요청의 401은 폼 오류로 보여줍니다 |
| `ROLE_NOT_ALLOWED` | 403 | 교사·학부모 역할이 맞지 않음 | "접근 권한 없음" 화면 |
| `CLASS_ACCESS_DENIED` | 403 | 담당이 아닌 반 | "접근 권한 없음" 화면 |
| `CHILD_ACCESS_DENIED` | 403 | 권한이 없는 원아 | "접근 권한 없음" 화면 |
| `CHILD_ACCESS_EXPIRED` | 403 | 졸업 후 열람 기간이 끝남 (NFR-03) | 화면에 남아서 안내 문구를 보여줍니다 |
| `INVALID_CURSOR` | 400 | 커서 값이 잘못됨 | 목록을 처음부터 다시 불러옵니다 |
| `VALIDATION_ERROR` | 422 | 요청 값 검증 실패 | 폼 오류로 보여줍니다 |
...
1. 이름·이메일·비밀번호
2. 어린이집 가입 코드 검증 결과 (FR-23)
3. 코드로 확인된 어린이집 이름
4. 코드가 없을 때의 직접 등록 입력값 — 어린이집 이름·주소 (FR-24)
5. 직접 등록 결과로 발급된 가입 코드
6. 이용약관·개인정보 처리방침 동의 여부
7. 가입 결과와 다음 단계

## 온보딩 1 — 반 선택 · 만들기

1. 코드로 확인된 어린이집 이름
2. 소속 어린이집의 기존 반 목록 (반 이름, 연령 구분, 담당 교사 유무) — 선택 경로 (FR-25)
3. 반 이름
4. 연령 구분 (만 0~2세 표준보육과정 6영역 / 만 3~5세 누리과정 5영역)
5. 우리 반 원아 수
6. 연령에 따른 영역 목록
7. 선택·생성 결과 (담당 반 식별자)
...
1. 원아 목록 (이름, 이니셜)
2. 원아별 법정대리인 동의 상태 (학부모가 앱에서 직접 동의한 결과, FR-28)
3. 원아별 등록된 사진 수
4. 등록 완료 인원 / 전체 인원
5. 동의 미등록 인원 수
...
> **09/22 결정: 동의는 앱 안에서 학부모가 합니다** — 원아별 초대 링크로 가입하며 직접 동의합니다 (FR-28).
> 이 화면은 교사가 그 결과를 보고 구분만 하며, 교사가 동의 상태를 입력하지 않습니다.
...
4. 선택 개수
5. 학부모 미리보기 (제목, 날짜, 반, 작성 교사 이름, 사진, 본문)
...
1. 접근 방식 — **원아별 초대 링크로 가입**하며 그 자리에서 동의 (09/22 결정, FR-28). 링크 발급·본인 인증 방식은 **(미정)**
2. 자녀 정보 (이름, 반)
...
| 모델 | 입력 | 출력 | 비고 |
| --- | --- | --- | --- |
| ~~Silero VAD (`@ricky0123/vad-web`)~~ | - | - | **2026-09-14 사용하지 않음.** 오디오를 자르지 않고 통째로 올리며 발화 구간 판정은 외부 STT 서비스에 맡깁니다 |
| ~~whisper-base STT (`@huggingface/transformers`)~~ | - | - | **2026-09-13 외부 STT 서비스로 이전.** 온디바이스에서 실행하지 않습니다 (파이프라인 1-B) |
...
| 소음 환경 STT 정확도 미검증 | 중 | 1주차에 80dB 교실 소음 환경에서 STT 실측, 데모에 실제 소음 환경 영상 포함. 09/13 외부 STT 서비스로 전환(실측은 그 서비스 기준으로 수행) |
| 외부 STT로 원아 실명 음성이 나감 | 상 | 09/13 STT 외부 전환에 따른 신규 항목. 제공자 선정 시 위탁 처리 계약·보관 정책(학습 이용 여부) 확인, 동의서 문구에 음성 처리 위탁 반영 |
...
- **결정 (2026-09-12): 동의받은 원아의 사진은 이미지 그대로 LLM에 전달합니다** (NFR-01, H-2).
- ~~결정 (2026-09-13): 미동의 원아가 섞인 사진은 그 얼굴만 블러한 사본을 올려 사본만 전달합니다.~~ → **09/13 저녁 리뷰 회의에서 폐기.** 블러 스파이크 결과 인식은 잘 되지만 블러 영역이 과도하게 커져 엉뚱한 곳까지 가렸습니다.
- **결정 (2026-09-13, 최종): 미동의 원아가 귀속된 사진은 행동 분석에 쓰지 않습니다.** 사진의 S3 업로드는 필수 동의(②)로 커버되어 그대로 하고, LLM 경로에서만 제외합니다. 미동의 원아는 얼굴로 식별되지 않으므로(③ 동의 미동의 = 임베딩 미등록) 교사 수동 분류로 귀속된 결과를 기준으로 판정합니다 (위 "공통 규칙").
...
- ~~**결론: 동의 수집·요청 절차만 앱 외부(서면, 교사↔학부모)로 뺍니다.**~~ → **2026-09-22 회의에서 뒤집힘.**
    - ~~교사가 서면 동의 결과를 앱에 등록하고(FR-01)~~ → 폐기. **학부모가 원아별 초대 링크로 가입하면서 앱에서 직접 동의합니다 (FR-28).** 링크로 반 참여를 유도하므로 교사가 대신 등록할 필요가 없습니다.
    - 시스템이 동의 상태로 원아를 구분하는 기능(FR-03)은 **그대로 유지**됩니다. 바뀐 것은 동의를 **받는 주체**뿐입니다.
    - 따라서 "앱이 학부모에게 동의를 요청하는 기능은 스펙 아웃"도 폐기입니다.
...
| ~~음성 구간 탐지 (VAD)~~ | ~~`@ricky0123/vad-web`~~ | **2026-09-14 채택하지 않음.** 오디오를 통째로 올리고 구간 판정을 외부 STT에 맡기기로 해서 브라우저 VAD가 불필요해짐 |
| STT (외부 서버) | 외부 STT 서비스 **(제공자 미정)** — 서버가 호출 | 2026-09-13 결정. 80dB 교실 소음 환경 정확도를 브라우저 whisper-base로 확보하기 어렵고, 모델 수십 MB 다운로드가 NFR-02(업로드 UX)와도 겹침. 오디오가 외부로 나가므로 위탁 처리 계약·동의 범위를 먼저 확인해야 함 |
...
flowchart TB
    ...
    MATCH -->|"귀속 결과 · 교사가 확정한 미디어"| API["FastAPI"]
    ...
    API --> STT
    STT -.->|"오디오"| EXTSTT["외부 STT 서비스"]
    ANON -->|"비식별 Evidence"| API
...
- **얼굴 인식만 vs 얼굴 + 호명 교차검증** — 09/15 **얼굴 인식 단독**을 선택. 호명은 얼굴과 산출 위치가 달라(온디바이스 vs 서버 STT) 합치는 비용이 크고, 애매한 건은 어차피 교사 수동 분류(FR-14)로 넘어갑니다. 호명 발화는 초안 작성 근거로 사용합니다
...
- **귀속 판정** — 온디바이스 얼굴 인식 테스트케이스 통과율 측정 (5-6주)
