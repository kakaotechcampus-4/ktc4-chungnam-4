...
| 엔티티 | 필드 | 설명/비고 |
| --- | --- | --- |
| MediaAsset(교사가 촬영해서 올린 원본 파일) | id, client_photo_id, class_id, teacher_id, type, captured_at, storage_url, storage_tier, redacted, model_version | client_photo_id는 클라이언트가 생성한 UUID로 UNIQUE 제약. 네트워크 실패로 재전송돼도 중복 저장되지 않음(`ON CONFLICT DO NOTHING`). redacted: 로컬 블러 처리 여부. **09/13 블러 폐기로 항상 false이며, 컬럼 유지 여부는 미정**입니다. model_version: 로컬 분류에 쓰인 모델 버전(교사 기기마다 다를 수 있음). storage_url의 객체 하나를 학부모 열람·다운로드(FR-09/10)와 초안 맥락 추출(NFR-09) 양쪽이 공유한다. 용도별 사본을 만들지 않는다. storage_tier: original/degraded/deleted. 보관 기한은 졸업 후 1년으로 확정(NFR-03, retention_expires_at). (미정) 기한 안에서 원본 → 저화질 전환 시점과 저화질 보관 방식(카카오톡 참고) |
| MediaChildLink(사진-아이 매칭 결과) | id, media_id, child_id, method, confidence_score | 교사 검수를 마친 확정 결과만 기록. 미분류·미검수 상태는 로컬에만 존재하며 서버로 전송되지 않음. method: 로컬 자동 / manual(로컬 수동). 09/15 호명 교차검증 폐기로 자동 분류는 얼굴 인식 단독이며, **자동 분류 쪽 값 이름은 확정 필요**(기존 `face_name_cross_check`는 더 이상 의미가 맞지 않음). 미식별 얼굴이 함께 찍힌 경우 교사 수동 분류로 넘기고, 교사가 귀속시키지 않은 인물의 행은 생성되지 않음 (09/13 블러 폐기) |
