...
| 1-A. 근거 수집 — 얼굴 | 온디바이스 모델 (코드가 호출) | MediaPipe BlazeFace 얼굴 검출 → ArcFace 임베딩 → 기준 임베딩과 비교 / frame_extract 근거 프레임 선택. 미동의·미식별 얼굴 블러도 여기서. 얼굴 실패 시 미분류함 | 아이별 얼굴 매칭 결과, 근거 프레임 | H-3. 얼굴은 온디바이스 유지 |
| 1-B. 근거 수집 — 음성(STT) | **외부 STT 서비스 (서버가 호출)** | 오디오(음성메모·영상 오디오)를 S3에 올린 뒤 서버가 외부 STT API를 호출해 발화 텍스트를 받아 `TranscriptSegment`로 저장. 인식 실패 시 "멘트 없음". **(미정)** 사용할 STT 제공자, VAD(발화 구간 탐지)를 온디바이스 전처리로 남길지 서버에서 할지 | 발화 텍스트 | 2026-09-13 결정: 온디바이스 whisper-base → 외부 STT. 소음 환경 정확도가 온디바이스 모델로는 확보되지 않아서. LLM에 가는 것은 3단계에서 비식별화된 텍스트라 H-2는 그대로 지켜집니다(오디오는 LLM에 가지 않음). 다만 **오디오 자체가 외부 처리자에게 나가므로 위탁 처리 계약·동의서 범위 확인이 필요**합니다 |
| 2. 귀속 판정 | 코드 (규칙 기반, **서버**) | 얼굴 매칭 결과(온디바이스 산출)와 발화 속 호명(서버 STT 산출)을 대조. 두 입력이 서로 다른 곳에서 나오므로 판정은 둘이 만나는 서버에서 수행. 신뢰도 임계값 미달·충돌 시 자동 결정하지 않음 | 아이별 귀속 결과, 미확정 건은 미분류함 | STT 서버 전환(1-B)에 따라 온디바이스 → 서버로 이동 |
...
| 6. 에이전트2·3 — 초안 생성 | LLM + tool (병렬) | 에이전트2: 영역 스타일 조회(표준보육과정 6대 영역)·발달 지침 조회 → 관찰일지. 에이전트3: 아이 감정 어휘 조회·발달 문안 조회 → 알림장 | 관찰일지·알림장 초안 2종 | FR-26: 작성 교사는 코드가 `author_teacher_id`로 기록. 교사 실명은 프롬프트에 넣지 않음 |
...
- **다인원(여러 아이가 함께 찍힌) 사진의 행동 분석** (2026-09-13 결정) — C 흐름 해당
  - 전원 동의: 사진 그대로 LLM에 전달합니다.
  - 미동의 원아 포함: 온디바이스에서 **미동의 원아 얼굴만 블러한 사본**을 만들어 그 사본을 업로드하고, LLM에는 사본만 전달합니다. 블러 전 원본은 서버로 올리지 않습니다 (H-3). 서버에 존재하는 객체가 블러본 하나뿐이므로 학부모 열람·다운로드(FR-09/10)에도 같은 블러본이 나갑니다.
  - **폴백**: 온디바이스 블러 구현이 어려우면 **다인원 사진을 행동 분석(LLM 근거)에서 통째로 제외**하고 1인 사진만 근거로 씁니다. 어느 경우에도 미동의 얼굴이 블러되지 않은 사진을 업로드하지 않습니다.
...
| 모델 | 입력 | 출력 | 비고 |
| --- | --- | --- | --- |
| Silero VAD (`@ricky0123/vad-web`) | 오디오 스트림(마이크 또는 영상 오디오 트랙, 16kHz PCM) | 발화 구간 목록 `[{start_ms, end_ms}]` | 무음 구간 제외. **(미정)** STT가 서버로 옮겨가면서(09/13) VAD를 온디바이스 전처리로 남길지 서버로 옮길지 다시 정해야 함 |
...
- **결정 (2026-09-13): 다인원 사진도 행동 분석에 씁니다.** 전원 동의면 그대로, 미동의 원아가 섞였으면 온디바이스에서 그 얼굴만 블러한 사본을 올려 사본만 전달합니다. 블러가 어려우면 다인원 사진을 행동 분석에서 제외하는 폴백을 씁니다 (위 "공통 규칙" 참고).
- **S3에 올라간 `MediaAsset` 원본을 그대로 씁니다. 맥락 추출용 사본을 따로 만들지 않습니다.** (미동의 얼굴 블러본은 예외가 아닙니다 — 블러본이 업로드되는 **유일한** 객체이고, 원본은 서버에 오지 않습니다.) 같은 객체가 학부모 열람·다운로드(FR-09/10)와 초안 근거(NFR-09) 양쪽에 쓰입니다. LLM 전달은 서버가 S3에서 읽어 보냅니다 — 브라우저가 LLM으로 직접 보내지 않습니다.
...
flowchart TB
    subgraph DEVICE["교사 브라우저 · 온디바이스"]
        CONSENT["0. 동의 확인"]
        FACE["1-A. 얼굴 검출·임베딩 비교<br/>미동의 얼굴 블러"]

        CONSENT --> FACE
    end

    FACE -->|"얼굴 매칭 결과 · 블러 완료 미디어"| API["FastAPI"]
