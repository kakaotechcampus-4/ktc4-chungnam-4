...
| 엔티티 | 필드 | 설명/비고 |
| --- | --- | --- |
| Job(`orchestrate_drafts` 요청 1건을 추적하는 레코드) | id, child_id, target_date, status, retry_count, created_at, updated_at | Celery task는 재시도마다 새 task ID를 받지만 같은 요청이면 Job은 하나. target_date는 EvidenceBundle.date를 채우는 데 씀. status: pending/running/succeeded/failed |
...
**공통 — 아직 정하지 않은 것**: 한 배치에 사진·영상·음성 혼재를 허용할지, 일지 생성 트리거 기준
