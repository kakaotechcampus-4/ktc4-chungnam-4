...
| should로 내림 | (미정) 학부모는 원아의 사진, 영상을 다운로드 가능함 | 서비스로서의 완성도 |
| should로 내림 | 선생님은 학부모가 확인할수 있는 공지를 게시판에 등록가능함 | 서비스로서의 완성도 |
| should로 내림 | 학부모는 선생님이 게시한 공지를 게시판에서 확인가능 | 서비스로서의 완성도 |
...
|  | (미정) 학부모는 원아의 사진, 영상을 다운로드 가능함 | 서비스로서의 완성도 |
|  | 선생님은 학부모가 확인할수 있는 공지를 게시판에 등록가능함 | 서비스로서의 완성도 |
|  | 학부모는 선생님이 게시한 공지를 게시판에서 확인가능 | 서비스로서의 완성도 |
...
| NFR-03 | (상세 내용 미확정)원아의 사진은 졸업 후 1년까지 원본으로 제공되며 이후 파기됨. | 서버비와 사용자 경험(내 아이의 추억이 사라졌어)의 균형 맞추기 |
...
| --- | --- | --- | --- | --- |
| 0. 등록 임베딩 캐시 조회 | 코드(서버 필터링) + 온디바이스(캐시)  | 배치 시작 시 서버가 학급의 등록된 원아 기준 임베딩만 브라우저로 전달(동의는 앱 외부에서 처리되며, 등록된 원아=이용 가능 원아). 브라우저는 이번 배치 동안만 메모리에 캐시. 임베딩 조회 실패 시 처리 중단 | 이번 배치용 기준 임베딩 캐시 |  |
...
| 5. 에이전트1 — 근거 수집 | LLM + tool | 발화 내용에 따라 tool 호출 (활동계획 조회 / 이전 기록 조회 / 원아 명단 조회). 근거 부족 시 재호출, 그래도 부족하면 미분류함 | 조합된 근거 묶음 |  |
...
| F-3 | 코드 | (미정) 원아 사진은 3개월간 원본 제공, 이후 원본 파기·저화질 전환 | NFR-03 |
...
- 미동의 아동 제외 방식 — FR-04는 '제외', 처리 파이프라인 0단계는 '얼굴 흐림 처리 후 제외'로 표현이 갈리며 구현 가능 여부도 논의 중
...
| EmbeddingLifecycleLog(생체정보 처리 이력) | id, child_id, device_id, event_type, created_at | event_type: register/re_register/device_change/device_revoked. device_id는 별도 참조 테이블 없이 식별값만 문자열로 기록. 벡터 값은 절대 포함하지 않는 순수 감사 로그 |
...
| DraftDocument(EvidenceBundle을 바탕으로 실제 생성된 문서 - 아이 한 명을 기준으로 알림장/일지 2건 생성) | id, child_id, evidence_bundle_id, doc_type, content, status, created_at, approved_at | doc_type: 관찰일지/알림장. status: draft→verified→approved. **status가 unclassified가 되면 UnclassifiedItem에 ref_type=draft인 행이 함께 생성됩니다**, FR-06 |
...
| 엔티티 | 필드 | 설명/비고 |
| --- | --- | --- |
| Notice(공지) | id, class_id, teacher_id, title, content, created_at | FR-11/12 미정 — 구현 확정 시 AccessLog 연동(학부모 열람 확인용) 필요 |
...
- **결론: 앱 내에서 동의/미동의 아동을 구분하는 로직은 스펙에서 제외.** 동의 절차는 앱 외부(교사↔학부모)에서 처리.
- 이에 따라 동의 관련 요구사항 항목 삭제. 단, 하위 문서에서 요구사항 ID로 참조하고 있으므로 **ID는 공란으로 남겨 두고 이후 일괄 재정렬**.
