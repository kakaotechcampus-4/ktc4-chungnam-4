...
| 엔티티 | 필드 | 설명/비고 |
| --- | --- | --- |
| ConsentRecord(학부모(법정대리인) 동의 기록 | id, child_id, parent_id, consent_type, status, agreed_at, revoked_at | consent_type: `개인정보수집이용`(①) / `활동사진영상촬영`(②) / `얼굴특징정보처리`(③) 동의 여부(사전 동의여부). **①②는 필수, ③은 선택**입니다 (09/13 등급 결정, 09/20 번호 확정 — 전제조건 참고). 번호는 화면·회의에서 부르는 이름이고 DB에는 문자열 값이 들어갑니다. 얼굴특징정보처리 미동의 원아도 서비스는 이용하며, 그 원아가 귀속된 사진만 LLM 근거에서 빠집니다. 법적 요구사항 문구는 요구사항 섹션 상단 이미지 기준으로 확정 필요 |
