...
def test_다른_원아의_근거를_참조하면_실패한다():
    document = _document(
        [
            DraftSentence(
                sentence_id="s_01", text="그림을 그렸다", evidence_ids=["ev_002"]
            )
        ]
    )
...
def test_대상_미확인_근거를_참조하면_실패한다():
    document = _document(
        [
            DraftSentence(
                sentence_id="s_01", text="미끄럼틀을 탔다", evidence_ids=["ev_004"]
            )
        ]
    )
...
def test_다른_날짜의_근거를_참조하면_실패한다():
    document = _document(
        [
            DraftSentence(
                sentence_id="s_01", text="그림책을 봤다", evidence_ids=["ev_005"]
            )
        ]
    )
...
def test_공동_활동_근거는_대상에_포함되면_통과한다():
    ...
    document = _document(
        [
            DraftSentence(
                sentence_id="s_01", text="함께 블록을 정리했다", evidence_ids=["ev_003"]
            )
        ]
    )
...
def test_문서와_근거가_같아도_요청_원아가_다르면_실패():
    document = _document(
        [DraftSentence(sentence_id="s1", text="그림", evidence_ids=["ev_002"])]
    )
