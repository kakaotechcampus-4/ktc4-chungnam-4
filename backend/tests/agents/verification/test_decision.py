...
DOCUMENT = DraftDocument(
    ...
    sentences=[DraftSentence(sentence_id="s1", text="블록 놀이", evidence_ids=["e1"])],
)
...
def results(failure=None):
    return [
        VerificationResult(
            ...
            stage=stage,
            passed=not (failure and stage == VerificationStage.CRITIC),
            issues=[VerificationIssue(check_type=failure, reason="검증 실패")]
            if failure and stage == VerificationStage.CRITIC
            else [],
        )
...
@pytest.mark.parametrize(
    "changes",
    [{"draft_id": "other"}, {"doc_version": 2}, {"doc_type": DocType.OBSERVATION_LOG}],
)
...
def test_실패한_코드검사는_Critic_없이_재생성_판정():
    ...
    checks[0] = VerificationResult(
        ...
        doc_type=DOCUMENT.doc_type,
        doc_version=DOCUMENT.version,
        stage=VerificationStage.REFERENCES,
        passed=False,
        issues=[
            VerificationIssue(
