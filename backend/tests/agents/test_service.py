    ...
    return contracts.DraftDocument(
        ...
        sentences=[
            contracts.DraftSentence(
                sentence_id="s1", text="블록 놀이를 했다", evidence_ids=list(evidence_ids)
            )
        ],
    )


def _request(evidence_ids: tuple[str, ...] = ("ev_001",)) -> contracts.GenerationRequest:
...
def test_verify_and_record_최종_판정을_draft_decision_log에_남긴다(monkeypatch) -> None:
    ...
    service._verify_and_record(
        session, document, EVIDENCE_BY_ID, _request(), regeneration_count=1, critic_retry_count=1
    )
...
def test_verify_and_record_재생성_상한에_도달하면_교사_확인으로_보낸다(monkeypatch) -> None:
    session = FakeSession()
...
def test_orchestrate_drafts_교사_확인이_필요하면_미분류함으로_보낸다(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        "_verify_with_critic_retry",
        lambda session, document, evidence_by_id, request, regeneration_count: (
            contracts.DecisionResult(decision=contracts.Decision.NEEDS_TEACHER_REVIEW, reason="테스트")
        ),
    )
    monkeypatch.setattr(
        service,
...
def test_orchestrate_drafts_재생성하면_이전_draft_id를_다시_넘기고_횟수를_늘린다(
    ...
    monkeypatch.setattr(
        service,
        "_send_to_unclassified",
        lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("재생성 성공 후엔 안 불러야 함")),
    )
