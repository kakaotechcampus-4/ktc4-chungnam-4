...
- **다인원 사진**은 전원 동의면 그대로, 미동의 원아가 섞였으면 온디바이스에서 그 얼굴만 블러한 사본을 전달합니다 (09/13 결정). 블러 여부는 `MediaAsset.redacted`로 확인하고, 확인되지 않은 사진을 LLM에 보내지 않습니다 (H-2). 블러 폴백이 확정되면 다인원 사진은 근거에서 제외합니다.
