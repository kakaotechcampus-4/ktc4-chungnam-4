...
def decide(
    ...
    *,
    ...
    if len(stages) != len(set(stages)) or any(
        ...
        != (document.draft_id, document.doc_type, document.version)
        for r in results
    ):
    ...
    if any(i.check_type == VerificationCheckType.CRITIC_RESPONSE_ERROR for i in issues):
        return DecisionResult(
            decision=Decision.RETRY_CRITIC,
            reason="Critic 응답 오류로 별도 재검사가 필요합니다.",
            issues=issues,
        )
    if failed:
        if regeneration_count < max_regenerations:
            return DecisionResult(
                ...
                reason="검증에 실패하여 해당 문서 재생성이 필요합니다.",
                issues=issues,
            )
        return DecisionResult(
            decision=Decision.NEEDS_TEACHER_REVIEW,
            reason="재생성 상한에 도달했습니다.",
            issues=issues,
        )
