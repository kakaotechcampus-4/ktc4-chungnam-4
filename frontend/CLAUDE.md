...
- **STT는 온디바이스가 아닙니다** (09/13 결정). 오디오는 S3에 올리고 서버가 외부 STT 서비스를 호출합니다 — 브라우저에서 whisper를 돌리지 마세요. VAD를 온디바이스 전처리로 남길지는 미정이라, 정해지기 전에 VAD 기준으로 오디오를 잘라 올리는 코드를 만들지 않습니다.
