...
**업로드 게이트 — 여기가 미동의 원아를 막는 유일한 방어선입니다**

- 분류→검수→전송이 독립적으로 움직입니다. `classify_state`(`pending`/`classified`/`unclassified`/`failed`)와 `review_state`(`미검수`/`확정`/`제외`)를 하나의 status로 합치지 마세요.
- **`review_state == 확정`인 사진만 업로드합니다.** 분류 실패분을 자동 전송하는 경로를 만들지 않습니다 — 교사가 로컬에서 직접 분류합니다.
- 제외는 `excluded_reason`(`미동의포함`/`부적절`/`기타`)을 남기고 전송하지 않습니다.
  <!-- 09/12 미동의 원아는 블러 후 업로드로 결정됨. 이 줄의 '미동의포함 = 전송 제외'와 겹치는지 확인 필요 → docs/open-questions.md -->
- `has_unidentified_face`가 참이면 `blob_redacted`(블러본)를 만들어 그쪽을 올립니다.
