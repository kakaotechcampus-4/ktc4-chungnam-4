...
- 코드 변경을 마치면 `npm run lint -- --fix`를 돌리고 결과를 보고합니다.
...
- **API 목록은 FE가 먼저 뽑습니다** (09/13 변경). 화면 흐름·피그마에서 확정·잠재 API 리스트를 내면 BE가 그걸 OpenAPI 계약으로 확정합니다. 리스트에 없는 화면을 만들기 전에 리스트를 먼저 고치세요.
...
- 얼굴 검출·임베딩 생성은 **여기서만** 일어납니다. 서버로 올라가는 것은 임베딩 벡터, 얼굴 매칭 결과, 업로드가 확정된 미디어입니다.
- **STT는 온디바이스가 아닙니다** (09/13 결정). 오디오는 S3에 올리고 서버가 외부 STT 서비스를 호출합니다 — 브라우저에서 whisper를 돌리지 마세요. **VAD도 쓰지 않습니다** (09/14 결정) — 오디오를 자르지 말고 통째로 올립니다.
- **영상·음성 메모는 로컬 처리 대상이 아닙니다** (09/14 결정). 프레임 추출·얼굴 분류·검수 없이 업로드 큐만 태워 S3로 보냅니다. `LocalPhoto`의 3축 상태는 **사진에만** 적용됩니다.
...
- **실패는 정상 흐름의 일부입니다**: 얼굴 실패 → 미분류함. `throw`로 파이프라인을 끊지 않습니다. (음성 실패 → `"멘트 없음"`은 서버 STT 쪽 규칙으로 옮겨갔습니다.)
...
- 제외는 `excluded_reason`(`부적절`/`기타`)을 남기고 전송하지 않습니다. **`미동의포함`은 더 이상 전송 제외 사유가 아닙니다** — 업로드는 하고 서버가 LLM 경로에서 빼는 것으로 09/13 정리됐습니다.
- **얼굴 블러는 하지 않습니다** (09/13 폐기 — 인식은 되지만 블러 영역이 과도하게 커집니다). `blob_redacted` 경로를 새로 만들지 마세요.
- `has_unidentified_face`가 참이면 블러 대신 **교사 수동 분류**로 보냅니다 (FR-14). 사진 자체는 업로드합니다 — S3 업로드는 필수 동의(1번) 범위입니다. 미동의 원아 보호는 서버가 LLM 경로에서 제외하는 것으로 합니다 (H-2).
- **다인원 사진 귀속 UI**: 한 명이라도 미등록·애매하면 사진 전체를 "미등록"으로 표시하고, **확실한 아이는 선택된 상태로 둔 채** 애매·미등록 아이만 교사가 고르게 합니다. 선택을 마치면 **선택된 모든 아이에게 귀속**됩니다 (09/13 결정).
