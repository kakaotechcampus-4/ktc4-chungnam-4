...
- **다인원 사진**: 전원 동의면 그대로 올리고, 미동의 원아가 섞였으면 **그 얼굴만 블러한 사본(`blob_redacted`)** 을 올립니다 (09/13 결정). 블러 없이 미동의 얼굴이 남은 사진을 올리는 경로를 만들지 마세요. 블러 구현이 어렵다고 판단되면 임의로 우회하지 말고 팀에 올리세요 — 폴백은 "다인원 사진을 행동 분석에서 제외"입니다.
